...
// CIGAM is MAGIC in the little endian format.
...
void dump_segment_commands(
  ...
  for (int i = 0; i < ncmds; i++) {
    struct load_command *cmd = load_bytes(
      obj_file,
      actual_offset,
    ...
    if (cmd->cmd == LC_SEGMENT_64) {
      struct segment_command_64 *segment = load_bytes(
        obj_file,
        actual_offset,
...
void dump_mach_header(
  ...
  if (is_64) {
    ...
    struct mach_header_64 *header = load_bytes(
      obj_file,
      offset,
      header_size
    );
    ...
    struct mach_header *header = load_bytes(
      obj_file,
      offset,
      header_size
    );
